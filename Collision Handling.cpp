...
/*
Improved by Presto-creator 12/10/2021
Functions improved to account for hashing collision: 
...
public:
	...
	string getInfo() {
		...
		return ("[" + to_string(bikeIdentificationNumber) + " , " + bikeName + "," + to_string(bikePrice) + " , " + currentOwner + " , " + currentOwnerAddress + " , " + currentOwnerPhoneNo + "]");
	}
...
void addNewbikeToCollection(Bicycle* newBicycle) {
	...
	while (hashTable[hashedLocation] != nullptr) // while the array location is taken up, iterate until found
	{
...
Bicycle* findBike(Bicycle* newBicycle) {
	...
	while (hashTable[hashedLocation]->getbikeIdentificationNumber() != newBicycle->getbikeIdentificationNumber()) //if the hashed location does not have the bike the function calls for, iterate until found
	{
		hashedLocation = (hashedLocation + 1) % 101;
	}
...
void printList(int size) {
	int i = 0;
	while (i < size) {
		cout << i << " : " << hashTable[i]->getInfo() << endl;   ++i;
	}
}
